...
def data_point_labels(dataset_index, iterable):
	"""
	Adds data labels as described here:
	  
	  http://code.google.com/apis/chart/labels.html#data_point_labels

	You will need to provide an iterable yielding with at least entries,
	they are:
		
	   (label_type, label_contents, color, data_point, size, priority)

	Optionally, you can also include an extra variable to set the
	`placement` of the label:

	  (label_type, label_contents, color, data_point, size, priority, placement)

	Keep in mind that flags and text labels require a very different 
	`label_contents` than a set of numbers.
	"""
	try:
		it = iter(iterable)
	except TypeError:
	...
	for m in it:
		try:
			label_type, label_contents = m[0:2]
			options = m[2:]
		except:
			continue
		label = smart_join("",
			label_types.get(label_type, label_type),
			label_contents
		)
		labels.append(smart_join(",", 
			label, options[0], dataset_index, *options[1:]
		))
	return {"chm": smart_join("|", *flatten(labels))}
